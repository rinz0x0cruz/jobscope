...
def run(cfg: dict, store) -> int:
    ...
    # only and never changes persisted score, tier, rationale, or resume routing.
    from jobscope.core import ai
    ai_on = ai.available(cfg, "seniority_advice") and match_cfg.get(
        "ai_seniority_tiebreak", True,
    )
    ...
    review_on = bool(match_cfg.get("ai_score_review", False)) and ai.available(
        cfg, "match_advice",
    )
    ...
    for job in jobs:
        ...
        if (ai_on and not was_blocked
                and required_experience_years(job) is None
                and (max_calls == 0 or ai_calls < max_calls)):
            ai_calls += 1
            c = classify(cfg, store, job)
            if c:
                seniority_advice.append(
                    f"  ~ {job.title} @ {job.company}: model suggests "
                    f"{c['level']} / ~{int(c['required_years'])}y"
                    + (f" / {c['discipline']}" if c.get("discipline") else "")
                )
    ...
    if seniority_advice:
        print(f"  local model produced {len(seniority_advice)} advisory seniority note(s):")
        for note in seniority_advice:
            print(note)
    if divergences:
