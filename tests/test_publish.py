...
def _seed(store):
    job = Job(
        ...
        company="Acme",
        url="https://jobs.example/acme-sse",
        is_remote=True,
        score=82.0,
        tier="Strong",
        resume_base="research",
        rationale="Strong overlap; ~1.7y experience (junior)",
    ).ensure_id()
    ...
    store.save_contacts(
        [
            Contact(
                ...
                source="team-page",
                profile_url="https://linkedin.com/in/dana-secret",
            )
        ]
    )
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with Store(cfg["output"]["db_path"]) as store:
            ...
            store.set_outreach_campaign_draft(
                target["id"],
                selected_email="campaign-canary@example.test",
                subject="Private campaign subject canary",
                body="Private campaign body canary",
            )

            # Full (local/encrypted) payload includes an allowlisted outreach snapshot.
            private = render.build_data(cfg, store, public=False)
            full = json.dumps(private, ensure_ascii=False)
        ...
        assert "Private campaign canary" in full
        assert "Campaign Only Corp" in full
        assert "campaign-canary@example.test" in full
        assert "Private campaign subject canary" in full
        assert "Private campaign body canary" not in full
        assert private["outreach_snapshot"]["read_only"] is True
        ...
        for secret in (
            ...
            "1.7y experience",
            "Private campaign canary",
            "Campaign Only Corp",
            "campaign-canary@example.test",
            "Private campaign subject canary",
            "Private campaign body canary",
        ):
            assert secret not in blob
        assert pub["outreach_snapshot"]["campaigns"] == []
