...
def _job(title: str, url: str) -> Job:
    return Job(
        ...
        is_remote=True,
        url=url,
        description="Python AWS security engineering",
    ).ensure_id()
...
def _fetch(status=ats.BoardStatus.OK, jobs=None, detail=""):
    return ats.BoardFetchResult(
        ...
        attempts=1,
        status_code=200
        if status in {ats.BoardStatus.OK, ats.BoardStatus.EMPTY}
        else 503,
    )


def test_supported_board_urls_and_arbitrary_careers_page_is_not_fetched(monkeypatch):
    assert ats.parse_board_url("https://job-boards.greenhouse.io/acme/jobs/123") == (
        "greenhouse",
        "acme",
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: (_ for _ in ()).throw(
            AssertionError("arbitrary careers page fetched")
        ),
    )
    ...
    assert resolution.status == ats.ResolutionStatus.UNRESOLVED
...
def test_scan_monitor_scores_links_reviews_and_records_health(monkeypatch):
    ...
    monkeypatch.setattr(
        ats,
        "fetch_company_result",
        lambda *_a, **_k: _fetch(
            jobs=[
                _job("Security Engineer", "https://x/one"),
                _job("Sales Manager", "https://x/two"),
            ],
        ),
    )
...
def test_scheduled_scan_archives_application_only_monitor(monkeypatch):
    ...
    called = []
    monkeypatch.setattr(
        monitor, "resolve_monitor", lambda *_a, **_k: called.append(True)
    )
...
def test_rescan_demotes_existing_match_and_excludes_pending_count(monkeypatch):
    ...
    monkeypatch.setattr(
        ats, "fetch_company_result", lambda *_a, **_k: _fetch(jobs=[fetched])
    )
    ...
    summary = next(
        item
        for item in store.company_monitor_summaries()
...
def test_scan_monitor_reports_decision_funnel(monkeypatch):
    ...
    monkeypatch.setattr(
        ats, "fetch_company_result", lambda *_a, **_k: _fetch(jobs=jobs)
    )
