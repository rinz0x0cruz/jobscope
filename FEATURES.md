...
| `outreach <job_id> [--to E] [--send] [--force]` | Drafts a tailored recruiter email + attaches your résumé and **previews it by default**. Resolves a contact deterministically — a real recruiter address from your mail, or a `careers@`/`jobs@` role inbox on a **confirmed** company domain (never guessed) — or asks for `--to`. Sending is opt-in (`apply.outreach.enabled` + `email.*` + `--send`), **deduped per company** with a cooldown, and honors a do-not-contact list. |
